page 50106 "Sanctioned v/s Vacant" //sai
{
        ...
        area(Processing)
        {
            action(Update)
            {
                ...
                begin
                    ...
                    rec.Rename("Sanctioned Post No.", 0);
                end;
            }
            /*   action(Attachments)
              {
            ...
            action(SubmitToHistory) //sai
            {
                ...
                begin
                    ...
                    IsSubmitted := true;
                    Rec.Modify(); // Save the record
                    Message('Sanctioned Strength submitted successfully.');
  ...
  // Handle document attachments
    ...
    begin
        if not IsSubmitted then begin
            Rec."Sactioned Strength" := 0; // Clear unsaved data
            Rec.Modify(); // or Rec.Rename() if needed
        end;
        exit(true);
    end;
