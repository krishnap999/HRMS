page 50106 "Sanctioned v/s Vacant" //sai
{
    ...
    actions
    {
        area(Processing)
        {
            ...
            action(SubmitToHistory) //sai
            {
                ...
                trigger OnAction()var DocumentAttachment, DocumentAttachment1: Record "Document Attachment";
                ...
                begin
                    ...
                    DocumentAttachment.SetRange("Line No.", rec."Approval No.");
                    ...
                    begin
                        ...
                        if SanctionedHistoryLrec.FindSet()then repeat if(SanctionedHistoryLrec."New Sanctioned Strength" = Rec."Sactioned Strength") and (SanctionedHistoryLrec."Department/Trade/Section" = Rec."Department/Trade/Section") and (SanctionedHistoryLrec.Designation = Rec.Designation) and (SanctionedHistoryLrec."Approval No." = Rec."Approval No.")then begin
                                    Inserted:=false;
                                end;
                        ...
                        if not Inserted then Error('Record with Entry No. %1 already exists.', SanctionedHistoryLrec."Entry No.");
                    end;
                end;
            }
            action(SanctionedPostHistory) //sai
            {
                ...
                trigger OnAction()var SanctionedPostHistoryLrec: Record "Sanctioned Post History";
    ...
    trigger OnInsertRecord(BelowxRec: Boolean): Boolean var DocAttach: Record "Document Attachment";
  ...
  // Handle document attachments
    ...
    trigger OnQueryClosePage(CloseAction: Action): Boolean var myInt: Integer;
    begin
    end;
    ...
    var myInt: Integer;
    sanc: Record "Sanctioned v/s Vacant";
    sanction: Page "Sanctioned v/s Vacant List"; //sai
    begin
        if booleanr = false then begin
            sanc.Reset();
            sanc.SetRange("Sanctioned Post No.", Rec."Sanctioned Post No.");
            if sanc.FindFirst()then sanc."Sactioned Strength":=0;
            Clear(sanction);
            sanc.Modify();
        end;
    end;
